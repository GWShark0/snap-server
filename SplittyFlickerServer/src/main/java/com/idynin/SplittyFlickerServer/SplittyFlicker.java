...
public class SplittyFlicker
{
	static int splitToFlick(File inputImage, File outputVideo, int fps, int msLen,
		...
		while (--imagesNeeded > 0) {
			...
			BufferedImage qr = toBufferedImage(makeColorTransparent(qrGen(inputImage.getName()
					+ "_" + System.currentTimeMillis()),
					Color.WHITE,
					(short) 0));

			BufferedImage qrTrans = new BufferedImage(qr.getWidth(), qr.getHeight(),
					BufferedImage.TYPE_INT_ARGB);
			Graphics2D qrG = qrTrans.createGraphics();
			AlphaComposite composite = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.04f);
			qrG.setComposite(composite); // Set current alpha
			qrG.drawImage(qr, 0, 0, null);

			for (int i : used) {
				tempG.drawImage(qrTrans,
						((i % rows) * chunkWidth) + (chunkWidth / 3),
						((i / cols) * chunkHeight) + (chunkHeight / 3),
						null);
			}
